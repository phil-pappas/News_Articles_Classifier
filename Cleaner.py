...
class Cleaner:
    ...
    # used??or not?
    def concatenate_dataframes(self, df1, df2, df3):
        frames = [df1, df2, df3]
    ...
    def drop_duplicate_values(self, df):
        df.drop_duplicates(subset=['Text'], keep='last', inplace=True)
        return df

    def remove_new_line(self, df):
        df.Text.replace('\n', ' ', regex=True, inplace=True)
        return df

    def remove_multiple_spaces(self, df):
        df.Text.replace('\s+', ' ', regex=True, inplace=True)
        return df

    def remove_non_ascii_chars(self, df):
        df.Text.replace(r'[^\x00-\x7F]', ' ', regex=True, inplace=True)
        return df

    def remove_most_noisy_words(self, df):
    ...
    def remove_ghost_words(self, list_2d):
    ...
    def remove_single_chars(self, df):
    ...
    def remove_double_chars(self, df):
    ...
    def remove_punctuation_v2(self, df):
